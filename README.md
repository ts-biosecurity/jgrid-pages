...
|---|---|---|---|
| Myanmar | Google News, WHO DON | **GNLM**（国営紙 Global New Light Of Myanmar の RSS）<br>**WHO Myanmar / SEARO ニュース**（who.int/myanmar 系を HTML スクレイプ）<br>**WHO SEARO Epidemiological Bulletin**（隔週 PDF 速報） | `myanmar_infectious_diseases.json`<br>`myanmar_gnlm.json`<br>`myanmar_who.json`<br>`who_searo_epi.json`<br>`who_don.json` |
...
各記事には `dataSource` フィールド（例: `"Google News"`, `"GNLM"`, `"WHO Myanmar"`）が付与されており、フロントエンド側ではこの値でソースバッジを色分けしフィルタを提供する。
...
- データソースバッジ `.source-badge.*`（`gnews` / `gnlm` / `who` / `default`）の bg/color
...
- [`myanmar/data_pipeline.md`](myanmar/data_pipeline.md) — Myanmar のデータパイプライン詳細（収集経路・Cloudflare 回避・SEARO Epi 統合の経緯）
